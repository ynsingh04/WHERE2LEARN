...
public class ProfessionalCourseWebServiceCall {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					callWebService(AppConfig.URIProfessionalCourseType);
			...
			public void onDismiss(DialogInterface dialog) {
				...
				if (isError) {
					...
					Intent intent = new Intent(activity,
							ProfessionalCourseActivity.class);
	...
	private void initArrayList() {
		...
		courseFilter = AppConfig.ProfessionalFilterModel.getCourseFilter();
	...
	private void parseSchoolXML(Object result) throws JSONException {
		...
		courseFilter.add(activity.getString(R.string.select_all));
		...
		for (int i = 0; i < cast.length(); i++) {
			...
			if (Search_Name.equalsIgnoreCase("Courses")) {

				courseFilter.add(Value);
