...
public class SendCollgeFiltersWebServiceCall {
	...
	private CollegeListActivity collegeListActivity;

	public void sendCollgeFilters(Activity activityParam, String city,
			String course) throws Exception {

		this.activity = activityParam;

		SendCollgeFiltersWebServiceCall.city = city;
		SendCollgeFiltersWebServiceCall.course = course;
...
//					activity.finish();
	...
	public synchronized void callWebService() throws Exception {
		...
		String URI = AppConfig.URI_POSTCOLLEGESEARCH + "?city=" + URLEncoder.encode(city, "UTF-8")
...
//		AppConfig.CURRENTRANGE += AppConfig.LISTRANGE;
	...
	public void loadCollegeFilters(CollegeListActivity collegeListActivityParam) {


		this.collegeListActivity = collegeListActivityParam;

		ringProgressDialog = ProgressDialog.show(collegeListActivity, "Please wait ...",
		...
		ringProgressDialog.setOnDismissListener(new OnDismissListener() {
			...
			public void onDismiss(DialogInterface dialog) {
				...
				if (isError) {
					...
					collegeListActivity.initListView();
				}
