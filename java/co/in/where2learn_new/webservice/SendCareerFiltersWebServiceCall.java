...
public class SendCareerFiltersWebServiceCall {
	...
	private CareerlListActivity careerlListActivity;

	public void sendCareerFilters(Activity activityParam, String city,
			String areaList, String categoryList, String subjectList) throws Exception {

		this.activity = activityParam;

		SendCareerFiltersWebServiceCall.city = city;
		SendCareerFiltersWebServiceCall.areaList = areaList;
		SendCareerFiltersWebServiceCall.categoryList = categoryList;
		SendCareerFiltersWebServiceCall.subjectList = subjectList;
...
//					activity.finish();
	...
	public synchronized void callWebService() throws Exception {
		...
		String URI = AppConfig.URI_POSTCAREERSEARCH + "?city=" + URLEncoder.encode(city, "UTF-8")
				+ "&Area=" + URLEncoder.encode(areaList, "UTF-8")
				+ "&Category=" + URLEncoder.encode(categoryList, "UTF-8")
				+ "&Subjects=" + URLEncoder.encode(subjectList, "UTF-8")
				+ getRange();
...
//		AppConfig.CURRENTRANGE += AppConfig.LISTRANGE;
	...
	public void loadCareerFilters(CareerlListActivity careerlListActivityParam) {


		this.careerlListActivity = careerlListActivityParam;

		ringProgressDialog = ProgressDialog.show(careerlListActivity, "Please wait ...",
		...
		ringProgressDialog.setOnDismissListener(new OnDismissListener() {
			...
			public void onDismiss(DialogInterface dialog) {
				...
				if (isError) {
					...
					careerlListActivity.initListView();
				}
