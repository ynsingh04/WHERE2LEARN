...
public class SendWorkShopFiltersWebServiceCall {
	...
	private static String calendarMonthList;
	private static String calendarYearList;
	...
	public void sendWorkShopFilters(Activity activityParam, String city,
			String calendarMonthList, String calendarYearList) throws Exception {

		this.activity = activityParam;

		SendWorkShopFiltersWebServiceCall.city = city;
		SendWorkShopFiltersWebServiceCall.calendarMonthList = calendarMonthList;
		SendWorkShopFiltersWebServiceCall.calendarYearList = calendarYearList;
...
//					activity.finish();
	...
	public synchronized void callWebService() throws Exception {
		...
		calendarMonthList = calendarMonthList.replace("Jan", "1");
		calendarMonthList = calendarMonthList.replace("Feb", "2");
		calendarMonthList = calendarMonthList.replace("Mar", "3");
		calendarMonthList = calendarMonthList.replace("Apr", "4");
		calendarMonthList = calendarMonthList.replace("May", "5");
		calendarMonthList = calendarMonthList.replace("Jun", "6");
		calendarMonthList = calendarMonthList.replace("Jul", "7");
		calendarMonthList = calendarMonthList.replace("Aug", "8");
		calendarMonthList = calendarMonthList.replace("Sep", "9");
		calendarMonthList = calendarMonthList.replace("Oct", "10");
		calendarMonthList = calendarMonthList.replace("Nov", "11");
		calendarMonthList = calendarMonthList.replace("Dec", "12");
		
		Log.e(TAG, calendarMonthList);
//		Toast.makeText(activity, calendarMonthList, Toast.LENGTH_LONG).show();
		String URI = AppConfig.URI_POSTWORKSHOPSEARCH + "?city=" + URLEncoder.encode(city, "UTF-8")
				+ "&month=" + URLEncoder.encode(calendarMonthList, "UTF-8")
				+ "&year=" + URLEncoder.encode(calendarYearList, "UTF-8")
				+ getRange();
...
//		AppConfig.CURRENTRANGE += AppConfig.LISTRANGE;
	...
	private void parseAreaXML(Object result) throws JSONException {
		...
		for (int i = 0; i < cast.length(); i++) {
			...
			itemBean.setStartDateAndTime(startDate);
			itemBean.setEndDateAndTime(endDate);
	...
	public void loadWorkShopFilters(WorkShopListActivity workShopListActivityParam) {


		this.workShopListActivity = workShopListActivityParam;

		ringProgressDialog = ProgressDialog.show(workShopListActivity, "Please wait ...",
		...
		ringProgressDialog.setOnDismissListener(new OnDismissListener() {
			...
			public void onDismiss(DialogInterface dialog) {
				...
				if (isError) {
					...
					workShopListActivity.initListView();
				}
