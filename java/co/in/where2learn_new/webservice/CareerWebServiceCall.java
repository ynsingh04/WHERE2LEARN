...
public class CareerWebServiceCall {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					callWebService(AppConfig.URICareerType);
		...
		ringProgressDialog.setOnDismissListener(new OnDismissListener() {
			...
			public void onDismiss(DialogInterface dialog) {
				...
				if (isError) {
					...
					Intent intent = new Intent(activity, CareerActivity.class);
	...
	private void initArrayList() {
		...
		categoryFilter = AppConfig.CareerFilterModel.getCategory();
		subjectFilter = AppConfig.CareerFilterModel.getSubject();
	...
	private void parseSchoolXML(Object result) throws JSONException {
		...
		for (int i = 0; i < cast.length(); i++) {
			...
			else if (Search_Name.equalsIgnoreCase("Subject")) {
