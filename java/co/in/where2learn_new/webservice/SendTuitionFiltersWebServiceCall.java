...
public class SendTuitionFiltersWebServiceCall {
	...
	private TuitionListActivity tuitionListActivity;

	public void sendTuitionFilters(Activity activityParam, String city,
			String typeList, String areaList, String classList,
			String boardList, String subjectList) throws Exception {

		this.activity = activityParam;

		SendTuitionFiltersWebServiceCall.city = city;
		SendTuitionFiltersWebServiceCall.typeList = typeList;
		SendTuitionFiltersWebServiceCall.areaList = areaList;
		SendTuitionFiltersWebServiceCall.classList = classList;
		SendTuitionFiltersWebServiceCall.boardList = boardList;
		SendTuitionFiltersWebServiceCall.subjectList = subjectList;
...
//					activity.finish();
	...
	public synchronized void callWebService() throws Exception {
		...
		String URI = AppConfig.URI_POSTTUITIONSEARCH + "?city=" + URLEncoder.encode(city, "UTF-8")
				+ "&Area=" + URLEncoder.encode(areaList, "UTF-8")
				+ "&Class=" + URLEncoder.encode(classList, "UTF-8")
				+ "&Board="	+ URLEncoder.encode(boardList, "UTF-8")
				+ "&Subject=" + URLEncoder.encode(subjectList, "UTF-8")
				+ "&Type=" + URLEncoder.encode(typeList, "UTF-8")
				+ getRange();
...
//		Toast.makeText(activity, "resultString: " + resultString, Toast.LENGTH_LONG).show();
	...
	public void loadTuitionFilters(TuitionListActivity tuitionListActivityParam) {

		this.tuitionListActivity = tuitionListActivityParam;

		ringProgressDialog = ProgressDialog.show(tuitionListActivity, "Please wait ...",
		...
		ringProgressDialog.setOnDismissListener(new OnDismissListener() {
			...
			public void onDismiss(DialogInterface dialog) {
				...
				if (isError) {
					...
					tuitionListActivity.initListView();
				}
