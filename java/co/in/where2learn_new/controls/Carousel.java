package co.in.where2learn_new.controls;
		...
		public void SetImages(TypedArray array, TypedArray names, boolean reflected){
			...
			for(int i = 0; i< array.length(); i++)
			{
				...
				if(reflected){
					...
					/** Edit for reflection removal */
//					canvas.drawBitmap(reflectionImage, 0, height + reflectionGap,
//							null);
					canvas.drawBitmap(reflectionImage, 0, height + height + reflectionGap,
							null);					
...
//		{
//			performItemClick(mSelectedChild, mSelectedPosition, mSelectedChild.getId());
			
            performItemClick(getChildAt(selectedIndex), selectedIndex, mAdapter
                    .getItemId(selectedIndex));
			return true;
//		}			
...
//        return false;
    ...
    private void Calculate3DPosition(CarouselItem child, int diameter, float angleOffset){
    	...
    	float y = - getHeight()/4 + (float) (z * android.util.FloatMath.sin(mTheta)) + 22;
